...
execute if score #bb.ae.rotation_threshold bb.result = #bb.ae.bool.true bb.enum if score #bb.ae.invisible bb.result = #bb.ae.bool.false bb.enum run scoreboard players operation @e[tag=boomber.armor_stand.vector, limit=1, sort=nearest] bb.ae.state = #bb.ae.selector.head bb.enum
