...
execute if score rotation_threshold bb.ae.result = #bool.true bb.ae.enum if score invisible bb.ae.result = #bool.false bb.ae.enum run scoreboard players operation @e[tag=boomber.armor_stand.vector, limit=1, sort=nearest] bb.ae.state = #selector.head bb.ae.enum
